...
class VPDEncoder(nn.Module):
    ...
    def forward(self, x, class_ids=None):
        ...
        class_embeddings = [class_embedding.to(device) for class_embedding in self.class_embeddings]

        c_crossattn = self.text_adapter(latents, class_embeddings, self.gamma.to(device))

        c_crossattn = c_crossattn.repeat(x.shape[0], 1, 1).to(device)
        ...
        for name, x in enumerate(feats):
            m = torch.zeros(x.shape[0], x.shape[2], x.shape[3], dtype=torch.bool, device=device)
...
class TextAdapterDepth(nn.Module):
    ...
    def forward(self, latents, texts, gamma):
        ...
        texts = texts[0].unsqueeze(0).to(device)
        texts_after = self.fc(texts)
        texts = texts + gamma * texts_after
        texts = repeat(texts, 'n c -> n b c', b=1).to(device)
        return texts
...
def register_hier_output(model):
    ...
    def forward(x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if self.num_classes is not None:
            ...
            emb = emb + self.label_emb(y.to(device))

        h = x.type(self.dtype).to(device)
        for module in self.input_blocks:
            module.to(device)  # Ensure module is on the same device
            h = module(h, emb, context)
            hs.append(h)
        self.middle_block.to(device)  # Ensure middle_block is on the same device
        h = self.middle_block(h, emb, context)
        out_list = []

        for i_out, module in enumerate(self.output_blocks):
            module.to(device)  # Ensure module is on the same device
            h = th.cat([h, hs.pop()], dim=1).to(device)
            h = module(h, emb, context)
            if i_out in [1, 4, 7]:
                out_list.append(h)
        h = h.type(x.dtype).to(device)
